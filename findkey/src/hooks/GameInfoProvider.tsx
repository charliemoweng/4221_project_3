...
const GameInfoProvider = ({ children }: Props) => {
  ...
  const fightOpponent = (team: Set<string>): Set<string> => {
    ...
    setMatchInfo({
      ...
      closuresOfSetsUsed: matchInfo.closuresOfSetsUsed.set(teamStr, closureStr),
   ...
   */
  const isValidKey = (newKey: string[], existingKeys: string[][]): boolean => {
  ...
  const generateBasicFDs = (
    ...
    candidateKeys.forEach((candidateKey) => {
      ...
      while (remainingAttributes > 0) {
        const subsetSize = Math.ceil(remainingAttributes / remainingFDs);
        const dependent = nonCandidateAttributes.slice(0, subsetSize);
        nonCandidateAttributes = nonCandidateAttributes.slice(subsetSize);
  ...
  const generateAugmentedFDs = (
    ...
    basicFDs.forEach((fd) => {
      ...
      const determinantSize =
        Math.floor(Math.random() * (determinantAttributes.length - 1)) + 1;
          ...
          if (!processedFDs.has(fd2)) {
            if (index1 !== index2) {
              ...
              if (lhs2.includes(lhs1) && !lhs2.includes(rhs1)) {
                ...
                let newLHS = lhs2
                  .split("")
                  .filter((char) => char !== lhs1 && !rhs1.includes(char))
                  .concat(rhs1.split(""))
                  .sort()
                  .join("");
                let newRHS = rhs2;
  ...
  const generateFDs = (
    ...
    const noOfFDs: number =
      Math.floor(
        Math.random() * (difficultyInfo.max_FDs - difficultyInfo.min_FDs + 1)
        ...
        for (let i = 0; i < redundantFDs; i++) {
          ...
          determinant.push(
            nonCandidateAttributes[
              Math.floor(Math.random() * nonCandidateAttributes.length)
            ]
          );
          const dependents: string[] = attributes.filter(
            (attr) =>
              !determinant.includes(attr) && !candidateKeyAttributes.has(attr)
          );
          fds.add([
            determinant.join(""),
            dependents[Math.floor(Math.random() * dependents.length)],
          ]);
        }
      });
    }
  ...
  const handleNewMatch = (difficulty: number) => {
    ...
    const noAttributes =
      Math.floor(
        Math.random() *
          (difficultyInfo.max_attributes - difficultyInfo.min_attributes + 1)
